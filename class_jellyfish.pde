class Jellyfish {
  ...
  float maxSpeed = 0.8;
  float currentHeading; //body orientatuion
  float turnRate = 0.05;
  float size;
  float depth; //fixed
  ...
  Jellyfish() {
    ...
    //each tentacle get random offsets
  ...
  void update(){
    ...
    global = contractionAt(phase);
    float dc = global - lastContraction;
    lastContraction = global;

    offsetY = lerp(offsetY, (0.5 - global) * 20, 0.15);
...
//user interaction
  ...
  void drawTentacles() {
    ...
    float rim = contractionAt(phase - waveDelay);
    float rimXZ = (1.0 - 0.40 * rim) * sin(bellAng);
    float rimY  = (-cos(bellAng) + tuck * rim) * bellSquash;
