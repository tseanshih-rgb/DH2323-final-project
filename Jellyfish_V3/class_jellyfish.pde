class Jellyfish {
  ...
  void display() {
    ...
    float bellPulse = sin(t * PI + tOffset); //-1~1
  ...
  void update() {
    float thrust = max(-cos(t * PI + tOffset), 0);
    x += vx * (0.3 + thrust * 1.5);
    if (x >= width || x < 0) {
      vx *= -1;
  ...
  void drawTentacles(float bellScale) {
    ...
    for (int tentacleIndex = 0; tentacleIndex < tentacleCount; tentacleIndex++) {
      ...
      float phi1 = HALF_PI;
      float theta1 = (float) tentacleIndex / tentacleCount * TWO_PI;
    ...
    for (int tentacleIndex2 = 0; tentacleIndex2 < tentacleCount2; tentacleIndex2++) {

      float phi1 = HALF_PI; //all tentacles grow from equator
      float theta1 = ((float) tentacleIndex2 + 0.5) / tentacleCount2 * TWO_PI;

      float x1 = 50 * bellScale * sin(phi1) * cos(theta1);
      float y1 = 0;
      float z1 = 50 * bellScale * sin(phi1) * sin(theta1);
